...
function filterByCondition(arr, condition) {
  ...
  let arrResult = arr.filter(condition);
...
function createArray(...elements) {
  ...
  if (elements.length > 0) {
    arrResult = Array.of(...elements);
  } else if (elements.length === 0) {
    console.log("Не передано жодного елемента для об'єднання");
  }
  return arrResult;
...
function getElementAtIndex(arr, index) {
  ...
  if (arr.length > 0) {
    if (index < arr.length) {
      ...
      console.log("Індекс виходить за межі масиву.");
    }
  } else {
...
function findElementIndexes(arr, element) {
  ...
  if (Array.isArray(arr)) {
    if (arr.indexOf(element)) {
      listIndxs[listIndxs.length] = arr.indexOf(element);
    }
    if (arr.lastIndexOf(element) !== arr.indexOf(element)) {
      listIndxs[listIndxs.length] = arr.lastIndexOf(element);
    }
  } else {
    return [];
  }
...
function customIncludes(arr, element) {
  ...
  const filterFn = (value) => value === element;
